...
function renderMkgen(){
  const root=$('mkgen');
  if(!lastKey){
    root.innerHTML='<div class="mkgen-empty"><div class="mkgen-empty-cols"><div class="mkgen-empty-col"><div class="mkgen-empty-col-title">Mining Key</div><div class="mkgen-empty-col-desc">Use the bare <span class="mono">&lt;40hex&gt;[.worker]</span> mining key in your miner. The pool derives bech32 payout addresses for SegWit-active BlakeStream chains.</div><button id="genKey" class="primary">Generate Mining Key</button></div><div class="mkgen-empty-col"><div class="mkgen-empty-col-title">Miner Username</div><div class="mkgen-empty-col-desc">Baikal and GPU miners use <span class="mono">&lt;miningkey&gt;.rig1</span> as the username and <span class="mono">x</span> as the password.</div></div></div></div>';
  } else {
    ...
    const bbtcActive=!!chainByTicker('BBTC').segwit_active;
    const bbtcPresent=rows.some(([label,d])=>ticker(label)==='BBTC'||d.hrp==='bbtc');
    root.innerHTML='<div class="mkgen-result"><div class="mkgen-cols"><div class="mkgen-col mkgen-col-left">'
      ...
      + ((!bbtcActive&&!bbtcPresent)?'<div class="mkgen-warning">BBTC will appear here after its 15.21 SegWit activation; until then the pool uses its legacy fallback address.</div>':'')
