...
import config, cymysql, traceback
class usemysql:
    def __init__(self, **k) :
    ...
    def connect(self) :
        self.conn = cymysql.connect(host=self.settings['host'], port=3306, user=self.settings['user'], passwd=self.settings['passwd'], db=self.settings['db'])
