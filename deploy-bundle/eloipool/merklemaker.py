...
SupportedRules = ('csv', 'segwit', 'testdummy')
...
class merkleMaker(threading.Thread):
	...
	def makeCoinbase(self, height, coinbase_aux = None):
		now = int(time())
		...
		rv = bytes( (len(rv),) ) + rv
		aux_map = self.CoinbaseAux if coinbase_aux is None else coinbase_aux
		for v in aux_map.values():
			rv += v
		if len(rv) > 95:
			t = time()
	...
	def makeMerkleRoot(self, merkleTree, height, coinbase_aux = None):
		cbtxn = merkleTree.data[0]
		cb = self.makeCoinbase(height=height, coinbase_aux=coinbase_aux)
	...
	def getMRD(self, coinbase_aux = None):
		try:
		...
		if coinbase_aux is not None:
			# AuxPoW child payout identity is per solver, so work issuance may need
			# a miner-specific coinbase aux blob instead of the shared session map.
			(merkleRoot, merkleTree, cb) = self.makeMerkleRoot(merkleTree, self.currentBlock[1], coinbase_aux=coinbase_aux)
		(prevBlock, height, bits) = self.currentBlock
		return (merkleRoot, merkleTree, cb, prevBlock, bits, rollPrevBlk)
	
	def getMC(self, wantClear = False, coinbase_aux = None):
		if not self.ready:
			with self.readyCV:
