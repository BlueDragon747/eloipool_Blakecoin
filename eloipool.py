...
def makeCoinbaseTxn(coinbaseValue, useCoinbaser = True, prevBlockHex = None, witness_commitment = NotImplemented):
	...
	if useCoinbaser and hasattr(config, 'CoinbaserCmd') and config.CoinbaserCmd:
		...
		# Zero-subsidy devnet blocks legitimately have coinbaseValue == 0. In
		# that case a coinbaser that emits zero outputs should be treated as a
		# clean no-op, not a failure. Preserve the historical "strictly less
		# than coinbaseValue" rule only when there is a positive reward to split.
		if coinbased > coinbaseValue or (coinbaseValue > 0 and coinbased >= coinbaseValue):
...
def getGotworkCoinbaseAux(username):
	if not gotwork or not username:
		return None
	now = time()
	cacheEntry = _gotworkAuxCache.get(username)
	if cacheEntry and now <= cacheEntry[0] + _gotworkAuxCacheTTL:
		return cacheEntry[1]
	try:
...
if not hasattr(config, 'AllowShareDifficultyAboveNetwork'):
	config.AllowShareDifficultyAboveNetwork = False
...
def clampTarget(target, DTMode):
	...
	# Historical eloipool keeps share difficulty at or below upstream network
	# difficulty to avoid losing blocks. On Blakecoin devnet we intentionally
	# allow a harder local share target for ASICs because block candidates are
	# still submitted against networkTarget earlier in checkShare().
	if getattr(config, 'AllowShareDifficultyAboveNetwork', False):
		# Keep stratum share difficulty independent from the legacy getwork
		# target. GotWorkTarget exists for the old getwork compatibility path;
		# if we fold it into the vardiff floor here it silently drags ASIC
		# stratum users back down to diff-1 on devnet.
		target = target
	else:
...
def getTarget(username, now, DTMode = None, RequestedTarget = None):
	...
	minTarget = config.ShareTarget if getattr(config, 'AllowShareDifficultyAboveNetwork', False) else max(networkTarget, config.GotWorkTarget)
	if work <= config.DynamicTargetGoal:
		if now < lastUpdate + config.DynamicTargetWindow and (targetIn is None or targetIn >= minTarget):
...
def getBlockHeader(username):
	MRD = MM.getMRD(coinbase_aux=getGotworkCoinbaseAux(username))
	merkleRoot = MRD[0]
...
def getBlockTemplate(username, p_magic = None, RequestedTarget = None):
	if server.tls.wantClear:
		...
		wantClear = False
	MC = MM.getMC(wantClear, coinbase_aux=getGotworkCoinbaseAux(username))
...
def getStratumJob(jobid, username = None, wantClear = False):
	MC = MM.getMC(wantClear, coinbase_aux=getGotworkCoinbaseAux(username))
	(dummy, merkleTree, coinbase, prevBlock, bits) = MC[:5]
	now = time()
	workLog.setdefault(username, {})[jobid] = (MC, now)
	return (MC, workLog[username][jobid])

def getExistingStratumJob(jobid, username = None):
	wld = workLog[username][jobid]
	return (wld[0], wld)
...
def checkShare(share):
	...
	if 'data' in share:
		...
		try:
			...
			# Old/global jobs remain valid during rolling updates, but new AuxPoW
			# payout identity is keyed by the solving username so child templates
			# can vary correctly across miners.
	...
	blkhash = blakehash(data)
	# Bitcoin's traditional bdiff-1 floor (32 leading zero bits in BE display) is
	# meaningless on devnet where the network target is far below it. Skip the
	# floor when SkipBdiff1Floor=True is set in config.
	if not getattr(config, 'SkipBdiff1Floor', False):
		if blkhash[28:] != b'\0\0\0\0':
			print('H-not-zero (%s)' % (username,))
			raise RejectedShare('H-not-zero')
	blkhashn = LEhash2int(blkhash)
	
	global networkTarget
	parent_valid = blkhashn <= networkTarget
	logfunc = getattr(checkShare.logger, 'info' if parent_valid else 'debug')
	...
	# Forward candidate headers to the merged-mine proxy. This remains separate
	# from the parent-valid path above so a Blakecoin miss can still be submitted
	# for aux-only acceptance when one or more child targets are easier.
	if gotwork and blkhashn <= config.GotWorkTarget:
		try:
			...
			info['parent_valid'] = bool(parent_valid)
			info['parent_status'] = 'parent-accepted' if parent_valid else 'parent-rejected'
	...
	if blkhashn > workTarget:
		# On Blakecoin devnet the upstream network target can be easier than an
		# operator-chosen ASIC share target. A valid block solve must still count
		# as an accepted share even if it missed the harder local share floor.
...
if not getattr(config, 'NoInteractive', False):
